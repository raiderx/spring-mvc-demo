...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function() {
                $("#categoryTable thead td[sortableColumn]").click(function() {
                ...
                $("#categoryTable .activityFilter").change(function() {
                    $("#example_active").val($(this).val());
                    $("#filterForm").trigger("submit");
                })
                <c:if test="${criteria.example.active != null}">
                $("#categoryTable .activityFilter option[value='<c:out value="${criteria.example.active}"/>']").attr("selected", "selected");
                </c:if>
			});
		</script>
	</head>
	<body>
		...
		<div id="categoryTable" class="contentTable">
			<table>
				<thead>
					<tr>
						<td sortableColumn="name"> Name </td>
                        <td sortableColumn="description"> Description </td>
						<td> </td>
					</tr>
				</thead>
				...
				<tr>
					...
					<td align="right">
                        ...
                        <c:if test="${category.active ==false}">
                            <button onclick="restoreCategory(${category.id});"> Restore </button>
                        </c:if>
                    </td>
				</tr>
			</c:forEach>
